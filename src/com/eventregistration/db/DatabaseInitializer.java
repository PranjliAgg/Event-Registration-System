    ...
    private static void createTriggers(Connection conn) throws SQLException {
        try (Statement stmt = conn.createStatement()) {
            ...
            String[] triggerSQL = {
                ...
                "CREATE TRIGGER trg_restore_seats_on_cancel AFTER UPDATE ON REGISTRATIONS FOR EACH ROW BEGIN IF OLD.status = 'confirmed' AND NEW.status = 'cancelled' THEN UPDATE EVENTS SET available_seats = available_seats + 1 WHERE event_id = NEW.event_id; END IF; IF OLD.status = 'confirmed' AND NEW.status = 'cancelled' THEN UPDATE REGISTRATIONS SET status = 'confirmed' WHERE event_id = NEW.event_id AND status = 'waitlisted' ORDER BY registration_date ASC LIMIT 1; END IF; END",
